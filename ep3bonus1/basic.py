...
class Parser:
    ...
    def atom(self):
        ...
        # We still include the options of '+' and '-' inside the parse error,
        # because this method is called from factor only, and factor always
        # receives its error messages from atom.
    ...
    def bin_op(self, func, ops, r_func=None):
        if not r_func: r_func = func
...
class Interpreter:
    ...
    def visit_BinOpNode(self, node):
        ...
        if node.op_tok.type == TT_POW:
            return left.value.powed_by(right.value)
